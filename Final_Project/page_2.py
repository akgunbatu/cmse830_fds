...
st.write(missing_counts[missing_counts > 0]) #displaying all the missing values and their corresponding columns
...
- Dataset's missing value was reduced to half for Streamlit to run without any issues
""")
...
df_missing = df_new[df_new.isna().any(axis=1)] #separating the rows with  missing values from the dataset
df_complete = df_new[~df_new.isna().any(axis=1)] #separating the acomplete values from the dataset for later use
df_missing_sampled = df_missing.sample(n=8000, random_state=42) #randomly sampling the missing values to reduce the number to 8,000


df_reduced = pd.concat([df_complete, df_missing_sampled], ignore_index=True) #combining the sampled missing 
...
st.write("""
        - The most drastic change after the imputation is the standard deviation for engine horsepower.
        - However looking at the histogram above, we can observe that the distribution is not affected extremely.
        - For other features, the KNN imputation is a really good imputation and doesn't shift the data much.
        - In the future, for drivetrain and body type, we can extract these values from an API into our dataset.""")
#bullet points of what we observe from the imputation
